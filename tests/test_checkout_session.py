...
def test_create_price(
    mock_stripe_product_create, mock_stripe_price_create, subscription
):
    mock_stripe_product_create.return_value = MagicMock(id="prod_test")
    mock_stripe_price_create.return_value = {"id": "price_test"}

    result = checkout_session.create_price(subscription)
...
def test_get_payment_link(mock_create_payment_link, caplog):
    ...
    result = checkout_session.get_payment_link(mock_customer, True)
